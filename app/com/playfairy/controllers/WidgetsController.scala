...
import com.playfairy.models.WidgetRepoImpl
...
class WidgetsController @Inject() (reactiveMongoApi: ReactiveMongoApi) 
  ...
  def read(id: String) = Action.async {
    implicit request => {
      widgetRepo.select(BSONDocument(Id -> BSONObjectID(id)))
